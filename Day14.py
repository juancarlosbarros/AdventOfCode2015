...
def part2():
    ...
    for animal in datas:
        ...
        rest_time = animal[3]
        while time < tot_time:
            steps = min(flight_time, tot_time-flight_time)
            for _ in range(steps):
                time += 1
                dist += speed
                distances[animal[0]].append(dist)
            steps = min(tot_time-time, rest_time)
            distances[animal[0]] += steps*[dist]
            time += steps
    # print(distances)
    reverse_distances = [{animal[0]:distances[animal[0]][i] for animal in datas} for i in range(tot_time)]
